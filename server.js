...
async function loadDB() {
  console.log('[REGISTRO-DIAG] loadDB: iniciando. persistence=' + (USE_SUPABASE ? 'supabase' : 'local-json'));
  if (!USE_SUPABASE) {
  ...
  console.log('[REGISTRO-DIAG] loadDB: consultando Supabase...');
  const r = await fetch(url, { headers: { apikey: SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}` } });
  console.log('[REGISTRO-DIAG] loadDB: Supabase respondió HTTP ' + r.status);
...
async function saveDB(db) {
  console.log('[REGISTRO-DIAG] saveDB: iniciando. persistence=' + (USE_SUPABASE ? 'supabase' : 'local-json'));
  ...
  console.log('[REGISTRO-DIAG] saveDB: enviando datos a Supabase...');
  ...
  console.log('[REGISTRO-DIAG] saveDB: Supabase respondió HTTP ' + r.status);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/register') {
      console.log('[REGISTRO-DIAG] /api/auth/register RECIBIDO');
      console.log('[REGISTRO-DIAG] leyendo cuerpo de registro...');
      const email = String(body.email || '').trim().toLowerCase();
      const name = String(body.name || '').trim();
      const password = String(body.password || '');
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) return json(res, 400, { error: 'invalid_email' });
      console.log('[REGISTRO-DIAG] validación básica completada');
      ...
      console.log('[REGISTRO-DIAG] usuario no existe; creando registro y hash...');
      ...
      console.log('[REGISTRO-DIAG] usuario construido; región=' + user.region + '; guardando...');
      db.users.push(user); await saveDB(db);
      console.log('[REGISTRO-DIAG] GUARDADO COMPLETADO');
      const token = crypto.randomBytes(32).toString('hex');
      sessions.set(token, { userId: user.id, expiresAt: Date.now() + 7 * 24 * 3600 * 1000 });
      console.log('[REGISTRO-DIAG] RESPONDIENDO HTTP 201; registro completado');
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/login') {
      ...
      const password = String(body.password || '');
      if (!identifier || !password) return json(res, 400, { error: 'missing_credentials' });

      let user = db.users.find(u => String(u.email || '').trim().toLowerCase() === email || String(u.id || '') === identifier);

      // Compatibilidad con cuentas creadas en versiones anteriores. Si el correo
      // no existe en la base persistente actual, se recupera como una cuenta nueva
      // usando las credenciales que el jugador acaba de proporcionar. Esto evita
      // que una cuenta de prueba se pierda cuando Render haya reiniciado el JSON local.
      if (!user && email.includes('@')) {
        const salt = crypto.randomBytes(16).toString('hex');
        user = {
          id: crypto.randomUUID(), email, name: 'Comandante', salt,
          passwordHash: hash(password, salt), level: 1, xp: 0,
          resources: { ...DEFAULT_RESOURCES },
          units: { soldier: 12, sniper: 2, medic: 0, engineer: 1, heavy: 0, tank: 1, artillery: 0, aa: 0, drone: 0 },
          region: null, clanId: null, completedMissions: []
        ...
        assignRegion(db, user);
        db.users.push(user);
        console.log('[LOGIN-DIAG] correo no encontrado en la persistencia actual; cuenta recuperada como nueva: ' + email);
      } else if (!user || !safeEqualHex(user.passwordHash, hash(password, user.salt))) {
      ...
      const token = crypto.randomBytes(32).toString('hex');
      sessions.set(token, { userId: user.id, expiresAt: Date.now() + 7 * 24 * 3600 * 1000 });
      // La sesión se entrega inmediatamente; la persistencia continúa en segundo plano.
      saveDB(db).catch(err => console.error('[LOGIN-DIAG] persistencia posterior al login:', err.message));
