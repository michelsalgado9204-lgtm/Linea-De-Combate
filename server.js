...
function authUser(req, db) {
  ...
  if (!session || session.expiresAt < Date.now()) {
    ...
    const persisted = (db.users || []).find(u => u.sessionToken === token && Number(u.sessionExpiresAt || 0) >= Date.now());
    if (!persisted) return null;
    session = { userId: persisted.id, expiresAt: Number(persisted.sessionExpiresAt) };
...
async function realtimeAuthenticate(client, msg) {
  ...
  if (!session || session.expiresAt < Date.now()) {
    sessions.delete(token);
    const persisted = (db.users || []).find(u => u.sessionToken === token && Number(u.sessionExpiresAt || 0) >= Date.now());
    if (!persisted) { wsSend(client, {type:'error', message:'Sesión inválida o expirada'}); return false; }
    session = { userId: persisted.id, expiresAt: Number(persisted.sessionExpiresAt) };
    sessions.set(token, session);
  }
  const user = db.users.find(u => u.id === session.userId);
